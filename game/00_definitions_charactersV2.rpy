...
init -995 python:
    ...
    def get_combat_character_ids(include_hollow=True):
        ...
        Orden de prioridad:
        1) Resolver unificado del Hub Saga (si existe).
        2) Catálogo inyectado/activo del Hub Saga.
        3) CHARACTER_DATA local.
        """
        try:
            import renpy.store as S
        except:
        ...
        if S is not None:
            try:
                ...
                if callable(fn_roster):
                    ids = [str(x) for x in list(fn_roster(include_hollow, True) or []) if str(x)]
                    if ids:
                        return ids
            except:
                pass
