...
rg -n "def bs_apply_advanced_resource_effect|S\.bs_apply_advanced_resource_effect|stamina_drain_target|stamina_target_to_hp_self|hp_to_stamina_target" game/01B_BATTLE_STATE_FACADE.rpy >/dev/null
...
rg -n "precombat_resource_perks_v2|resource_perks_v2|shadow_target_mode|Perks de Recursos \(v2\)|precombat_legacy_specials_fallback_enabled = False" game/04I_PRECOMBAT_LOADOUT_SCREENV1.rpy >/dev/null
...
rg -n "resource_perks_v2|perks_v2|shadow_target_mode|applied_to_enemy|specials" game/04b_battle_startV2.rpy >/dev/null
